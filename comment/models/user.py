...
class User(db.Model):
    ...
    login_time=db.Colume(db.DateTime,default=datetime.now(),doc='登录时间')
    create_time = db.Colume(db.DateTime, default=datetime.now(), doc='用户注册时间')
    update_time = db.Colume(db.DateTime, default=datetime.now(), onupdate=datetime.now(),doc='用户修改时间')
